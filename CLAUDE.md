...
- **Two READMEs to keep in sync, with deliberate divergences.** The repo's top-level `README.md` (rendered on GitHub) and the `readme` key inside `Workflow/info.plist` (the in-Alfred description) carry the same usage prose — update both. Legitimate differences:
  - **Image paths.** The GitHub README uses `Workflow/images/...` and `Workflow/icon.png`; the in-Alfred copy uses bare `images/...` and `icon.png` because Alfred renders it relative to the workflow dir.
  - **Install / download affordances.** The GitHub README has a "⤓ Download latest release" link near the top pointing at `releases/latest/download/Tailscale.alfredworkflow`. The in-Alfred copy omits this — users reading it have already installed the workflow, so a download link is noise. Same logic applies to any future install/setup instructions.
